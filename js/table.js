...
function table() {
	...
	function chart(selector, data, tableId, columns,alphaReceived,betaReceived,gammaReceived){
		...
		for (i = 0; i < k; i++){	
			// Use edge width to represent edge weight
			tbody.select('tr:nth-child(' + (i+1) + ')').select('td:nth-child(2)').select('hr').style("height",weight1[i] * 1.5 + "px");
			tbody.select('tr:nth-child(' + (i+1) + ')').select('td:nth-child(4)').select('hr').style("height",weight2[i] * 1.5 + "px");
			tbody.select('tr:nth-child(' + (i+1) + ')').select('td:nth-child(6)').select('hr').style("height",weight3[i] * 1.5 + "px");
		}
		
		// Use luminance to represent the importance of that column
		rows.select('td:nth-child(2)').select('hr').style("background","hsl(240, 100%," + ((1 - alphaReceived) * 80 + 20) + "%)");
		rows.select('td:nth-child(4)').select('hr').style("background","hsl(240, 100%," + ((1 - betaReceived) * 80 + 20) + "%)");
		rows.select('td:nth-child(6)').select('hr').style("background","hsl(240, 100%," + ((1 - gammaReceived) * 80 + 20) + "%)");
